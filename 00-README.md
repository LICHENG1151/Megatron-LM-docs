...
| 12 | [训练可靠性工程](./reliability/00-README.md) | 文档集（效率+高可用+容灾+支撑）：**01 总览**(两问→三问·ETTR/MFU/Goodput·故障率随规模·Young/Daly·RTO/RPO·三层框架)、**02 高可用与容错**(源码锚定五维:资源/网络/状态/正确性/调度+横切:检测归因·故障演练)、**03 超出框架的工程考量**(相关性故障/可观测性/数值稳定/数据存储/规模化/电热/SRE/安全) |
| 99 | [面试题集](./interview/00-README.md) | 各子系统的面试/自测题集（含参考答案、Megatron 源码片段与源码定位）：[01 提问路线图](./interview/01-面试提问路线图与分级评分.md)、[02 并行化](./interview/02-并行化子系统面试题集.md)、[04 分布式训练与优化器](./interview/04-分布式训练与优化器面试题集.md)、[06 训练框架 Harness](./interview/06-训练框架Harness面试题集.md)、[08 检查点与重切分](./interview/08-检查点与重切分面试题集.md) |
