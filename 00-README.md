...
| 12 | [训练高可用与容错](./12-训练高可用与容错.md) | HA/FT 五维：①训练资源(进程内重启/弹性world size/慢节点)、②通信网络(四层栈:NCCL/网卡/fabric)、③状态持久化(副本ckpt/完整性校验/数据可恢复)、④计算正确性(SDC重算)、⑤调度弹性(ft_launcher)；横切:检测归因·故障演练 |
