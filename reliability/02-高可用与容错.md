...
> **完整需求心智图（一句话需求 → 三问主干 → 各维度 → 各特性 → 落点）见 [01 §0 完整心智图](./01-可靠性工程总览与Goodput目标.md)**——那是全集**唯一的完整全景**。本篇（02）是那张图里 **HA + 计算正确性 + 检测/演练** 支的**源码级展开**；DR 与超框架支撑见 [03](./03-超出框架的工程考量.md)。

本篇按上面 §0 的维度地图（①~⑤ + 横切 A/B）逐块展开。其中**三处是本轮新补的需求**（在 [01 §0](./01-可靠性工程总览与Goodput目标.md) 总表标 🆕），落点如下：

| 新补特性 | 本篇落点 | 一句话 |
|---------|---------|--------|
| 传输数据完整性校验 | §2.6 | 链路/传输/内存三级 bit 校验（FEC/ICRC/ECC），挡不住的位翻转 → SDC(§4) |
| 通信组打流监控 | §A.5 | per-PG 总线带宽/延迟**时序**观测，承接 §A.3 点状 sniff test |
| Pod/节点级重调度 | §5.0 阶梯定位 → 详见 [03 §C6](./03-超出框架的工程考量.md) | 三级重调度阶梯最上层 L3，属集群编排（超框架） |

> 正交主线 · **正确性三处校验**：计算 §4(重算) × 通信 §2.6(FEC/ICRC/ECC) × 存储 §3.3(SHA-256)——同一条"算得对"需求在三处各设一道校验，完整视角见 [01 §0](./01-可靠性工程总览与Goodput目标.md)。
...
> 前几节（2.2–2.4）问的是"链路断了怎么恢复"；本节问一个**正交**的问题——**链路没断，但数据在传输途中被翻了个 bit，结果错了却无人报错**。这是"通信正确性"，与 §4 的"计算正确性"、§3.3 的"存储正确性"共同构成贯穿全篇的 **正确性三处校验** 正交主线（见 §0.2）。

### 问题是什么

大规模 RDMA 网络里，光模块误码、电磁干扰、NIC/交换机 SRAM 偶发翻转，都可能让一份梯度/激活的某几个 bit 在传输途中被悄悄改写。若无校验，坏数据被 all-reduce 平均进全局梯度，表现为 loss 轻微异常甚至静默漂移——和 §4 的计算 SDC 一样隐蔽而难归因。

### 使用什么方法解决：三级校验（自底向上）

| 层 | 机制 | 覆盖 | 兜不住时 |
|----|------|------|---------|
| **链路层** | **FEC 前向纠错**（§2.3 已从"自愈"角度提过，此处换"完整性"视角） | 物理误码/瞬时抖动，就地纠正、不上抛 | 超纠错能力 → 上抛 ②传输层 |
| **传输层** | **RDMA ICRC**（InfiniBand invariant CRC，逐包硬件校验）/ 以太网 FCS·CRC；RoCE 另有 UDP checksum | 包级 bit 错，坏包丢弃 + 重传 | 永久错 → QP completion error 上抛 ①NCCL（呼应 §2.3） |
| **内存/缓冲** | GPU/HBM 与 NIC/交换机 buffer 的 **ECC**（SECDED：单 bit 纠、双 bit 检报） | 数据驻留期的 bit 翻转 | 双 bit 不可纠 → 报 uncorrectable，触发 §1.3 straggler 判定 / 坏卡下线 |

### 边界澄清：硬件校验之上，NCCL 默认不做端到端应用层校验

- RDMA ICRC 只保**逐包**完整性；跨多跳中转、经软件拷贝、或落在校验盲区（某些 SHM/PCIe 路径）的翻转，硬件 CRC **未必兜得住**。
- NCCL **默认不做**端到端应用层数据校验和（`NCCL_CHECKS` 是参数/一致性检查，**不是**逐字节数据校验）；开启数据校验有性能代价，通常不在训练热路径常开。
- 因此三级校验挡不住的位翻转，最终以 **SDC** 形态浮现 → 落到 §4 `RerunStateMachine` 换卡重算归因。这正是"**通信正确性**"与"**计算正确性**"的接力点：前者尽量在传输链路上就地拦截，拦不住的交给后者事后甄别。

### 三处校验对照（正交主线收口）

| 环节 | 正确性风险 | 校验手段 | 本篇位置 |
|------|-----------|---------|---------|
| **计算** | 硬件算错（SDC / NaN / spiky loss） | 重算并比对归因 | §4 |
| **通信** | 传输途中 bit 翻转 | FEC / RDMA ICRC / ECC | 本节 §2.6 |
| **存储** | ckpt 静默写坏（截断/半写/位翻转） | SHA-256 manifest | §3.3 verify_integrity |

> **路由到 03**：硬件层的 RAS 计数器采集、DCGM 的 ECC 指标体系、SDC 归因平台等"成体系遥测/归因"超出 Megatron 源码边界，见 [03 §B2 可观测性](./03-超出框架的工程考量.md)、[03 §B3 数值稳定与可复现](./03-超出框架的工程考量.md)。本节只覆盖"框架之下网络栈本身提供的确定性校验机制"。
...
>
> **正确性不止计算侧**：通信途中的 bit 翻转见 §2.6、存储的静默损坏见 §3.3，三者构成"正确性三处校验"正交主线（§0.2）；本部分聚焦**计算侧**，也是硬件 CRC/ECC 挡不住时位翻转最终浮现的兜底环节。
...
"重调度"不是一个动作，而是**由轻到重的三级阶梯**，每级换进程/资源的粒度与代价不同：

| 级 | 名称 | 机制 | 换什么 | 时延 | 归属 |
|----|------|------|--------|------|------|
| **L1** | 进程内重启 | `inprocess.Wrapper` 原地重组（§1.1） | 不换进程，只重跑训练函数 | 秒 | 本篇 §1.1 |
| **L2** | **进程级重调度** | `ft_launcher` 经 rendezvous 重拉进程（§5.1/5.2） | 换进程，节点不变 | 分钟 | 本篇 §5.1/5.2 |
| **L3** | **Pod / 节点级重调度** | k8s 重建 pod、drain/replace 坏节点、坏卡池换新（编排层） | 换 pod / 节点 | 分钟~十分钟 | **[超出框架] [03 §C6](./03-超出框架的工程考量.md)** |

- L1 → L2 → L3 是**逐级 fallback**：L1 救不了（进程已死）退 L2；L2 也救不了（节点死 / 无健康进程可集结）退 L3。
- L1/L2 能锚 Megatron + NVRx 源码，是本篇主体；**L3 属集群编排层、超出源码边界**，此处仅定位，详见 [03 §C6 作业编排与 Pod/节点级重调度](./03-超出框架的工程考量.md)。下面 §5.1–5.3 聚焦 L1/L2 的分工与备份来源。
...
3. **调度补位**：B 路径，`ft_launcher` 从共享空闲池经 rendezvous 补入新节点，下次集结扩回原规模（NCCL Grow API / 重新 rendezvous）。这条即三级阶梯的 **L3**，落到 pod/节点编排层的具体机制见 [03 §C6](./03-超出框架的工程考量.md)。
...
## A.5 通信组打流监控（per-PG 流量观测）

### 问题是什么

sniff test（A.3）是**点状探针**——某一时刻跨 rank 打一轮微基准，抓离群卡。但很多通信侧退化是**渐变/间歇**的：某条 rail 带宽缓慢下滑、某个 EP all-to-all 长期不均、PP P2P 偶发抖动。这些抓不到于一次快照，需要**常态化、按通信组（process group）分维度的流量/带宽·延迟时序观测**。

### 使用什么方法解决 / 如何实现（本篇可锚部分）

- **承接 A.3 并周期化**：`gpu_sniff_test` 已按 PG 分别打流——All-reduce=全局 PG 总线带宽、Reduce-scatter=TP PG、All-to-all=EP PG、Send/recv=DP PG；用 `--gpu-sniff-test-interval` 训练内周期触发，即得到 per-PG busbw 的**时序采样**。局限：缺 PP PG、粒度偏粗、是**主动打流**而非被动流量观测。
- **NCCL 侧可锚信号**：`NCCL_DEBUG=INFO` + `NCCL_DEBUG_SUBSYS=COLL,NET` 输出每次集合通信的算法/通道/耗时；**NCCL flight recorder（飞行记录仪）**留存最近通信轨迹，hang 时用于定位"卡在哪个 PG 的哪一步"（呼应 2.2 的 fail-fast + RAS）。
- **与 Profiler 交叉**：训练内通信时序的细粒度剖析见 [06 训练框架 Harness §7 Profiling](../06-训练框架Harness.md)（PyTorch Profiler / Nsys / NVTX），可按 PG 标注通信段。

### per-PG 该监控什么

| 通信组 | 主原语 | 该盯的指标 | 典型异常 |
|--------|--------|-----------|---------|
| 全局 / DP PG | All-reduce | 总线带宽 busbw | 某 rail 掉速 → 全局每步变慢 |
| TP PG | Reduce-scatter / All-gather | busbw、节点内 NVLink 利用 | 掉出 NVLink 域 → 带宽骤降 |
| EP PG | All-to-all | 各 rank 收发是否均衡 | 专家负载倾斜 → a2a 长尾 |
| PP PG | P2P send/recv | 逐段延迟、气泡占比 | 某 stage 抖动 → 整条流水线 stall |
| CP PG | All-gather / P2P(环形) | 环形延迟 | 长序列下带宽压力 |

（并行组"谁和谁通信"的语义见 [02.4 §2.5](../02.4-并行组构建与通信详解.md)，六并行通信对照总表见 [interview/02 §Q26](../interview/02-并行化子系统面试题集.md)。）

> **路由到 03**：把上述信号汇成**成体系的通信遥测平台**（Prometheus/Grafana per-PG 看板、flight recorder → 通信依赖追踪如 Mycroft、异常自动告警闭环）超出 Megatron 源码边界，正是 [03 §B2 可观测性](./03-超出框架的工程考量.md) 明确标注的缺口。本节只覆盖"框架内能打到的点"。
