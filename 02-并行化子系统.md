...
| [02.7 · 上下文并行与专家并行](./02.7-上下文并行与专家并行.md) | CP & EP | CP 切序列（Ring Attention/Ulysses、**因果掩码 Zigzag 负载均衡**）、EP 切专家（两次 All-to-All 路由）、五种并行通信特征总表 + 放置直觉 |

**③ 进阶专题**

| 子文档 | 主题 | 核心内容 |
|--------|------|----------|
| [02.8 · 进阶专题](./02.8-进阶专题.md) | 前沿权衡 | **Muon 分布式化**难题、**长序列工具箱**（Cut Cross-Entropy / 2D CP）、**FSDP vs TP+PP** 选型 |

> 阅读建议：**顺序读 02.0 → 02.8 即可**，前四篇（02.0–02.3）是与框架无关的铺垫，读完再进入 02.4 起的 Megatron 实现会非常顺。其中 **02.1.1（FlashAttention）、02.3.1（FSDP）是对应铺垫篇的技术专题**，走"背景→问题→数学→实现"，想深入时读、赶进度可先跳过。已有基础者可直接跳到对应并行维度。所有实现篇的通信组都源自 [02.4](./02.4-并行组构建与通信详解.md) 的 `parallel_state`。
