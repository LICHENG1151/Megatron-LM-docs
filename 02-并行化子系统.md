...
本篇是并行化子系统的总览，每个并行维度的实现细节拆分到以下子文档（按阅读顺序）：

| 子文档 | 主题 | 核心内容 |
|--------|------|----------|
| [02.0 · Transformer + MoE 结构与张量维度基础](./02.0-Transformer与MoE结构基础.md) | **前置基础** | 学并行前先读：一个 MoE Transformer 层逐步张量维度（单卡逻辑视角），**MHA 与 MLA 分轨**对比，并给出五种并行的「下刀点」地图 |
| [02.1 · 并行组构建与通信详解](./02.1-并行组构建与通信详解.md) | `parallel_state` / 通信组 | 分布式基础概念、`generate_masked_orthogonal_rank_groups` 分组算法、`RankGenerator`、`initialize_model_parallel` 建组三步、`order` 参数与拓扑映射 |
| [02.2 · 张量并行实现详解](./02.2-张量并行实现详解.md) | TP | 列/行并行切分、`f`/`g` 对偶算子、MLP/Attention/Embedding 的计算-通信顺序、序列并行、梯度/优化器状态/checkpoint 的获取-通信-保存 |
| [02.3 · 流水线并行与 1F1B 调度](./02.3-流水线并行与1F1B调度.md) | PP | 气泡问题、microbatch、1F1B 三阶段（warmup/steady/cooldown）、P2P 融合收发、交错式流水线 |
| [02.4 · 上下文并行与专家并行](./02.4-上下文并行与专家并行.md) | CP & EP | CP 切序列（Ring Attention/Ulysses）、EP 切专家（两次 All-to-All 路由）、五种并行通信特征总表 |

> 阅读建议：**先读 [02.0](./02.0-Transformer与MoE结构基础.md)** 建立"模型结构 + 张量维度"的直觉，再读 [02.1 §0](./02.1-并行组构建与通信详解.md) 的分布式预备知识，然后按 02.2→02.4 逐层叠加并行。已有基础者可直接跳到对应并行维度。所有子文档的通信组都源自 02.1 的 `parallel_state`。
