...
`T` = 流经某专家的 token 数（动态）。单专家的 `fc1→act→fc2` 就是一个标准 MLP（见 [02.5 §6](./02.5-张量并行实现详解.md)）。
...
|--------|--------------|------|:------:|------------------------------|------|
| **① Emb** | Embedding 出口 | All-Reduce | 前向 | 切**词表维**，每卡只查到一段 → 部分和求和 | [02.5 §4](./02.5-张量并行实现详解.md) |
...
| **③ MLP** | Dense-MLP 出口（fc2） | All-Reduce | 前向 | 行并行沿**收缩维**→ 部分和 | [02.5 §6](./02.5-张量并行实现详解.md) |
| **③a/③b MoE** | MoE dispatch / combine | All-to-All ×2 | 前/反向 | 专家分卡，token 须**去专家所在卡**算、再送回 | [02.5 §6.5](./02.5-张量并行实现详解.md)、[02.8 §3](./02.8-专家并行.md) |
| **④ Loss** | vocab_parallel_cross_entropy | All-Reduce ×3 | 前向 | logits 切词表维，softmax 需**全局 max 与 Σexp** | [02.5 §4](./02.5-张量并行实现详解.md) |
