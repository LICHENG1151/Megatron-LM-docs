...
把 §5.6（**WHEN**：通信落在前向/反向哪一段）与 §6（**WHERE**：落在哪个算子）两张表**画成一张竖版全景**：一次前向从 `token → loss`，每个算子旁标出它触发的集合通信。读者可一眼建立"完整数学直觉"——**哪里通信、通信什么、为什么**。逐块的代码与推导见 [02.5](./02.5-张量并行实现详解.md)（TP，同图也在其 §0）、[02.7](./02.7-上下文并行与专家并行.md)（CP/EP）。
...
| **③a/③b MoE** | MoE dispatch / combine | All-to-All ×2 | 前/反向 | 专家分卡，token 须**去专家所在卡**算、再送回 | [02.5 §6.5](./02.5-张量并行实现详解.md)、[02.7 §6](./02.7-上下文并行与专家并行.md) |
