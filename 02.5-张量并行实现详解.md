...
先给一张**竖版全景图**，把一次前向从 `token → loss` 的每个算子、以及**每个集合通信落点**都标出来，建立"完整数学直觉"。后面 **§1–§3** 先讲清 TP 的切法与 `f`/`g` 机制；**§4–§6.5 照这张图从上到下逐块展开**（Embedding → Attention → MLP → MoE）；§7 起再讲通信次数、序列并行、梯度、优化器、checkpoint 等机制。
...
|--------|--------------|------|:------:|------------------------------|----------|
| **① Emb** | Embedding 出口 | All-Reduce | 前向 | 切**词表维**，每卡只查到一段 → 部分和求和拼回 | [§4](#4-词嵌入并行vocabparallelembedding) |
...
| **③ MLP** | Dense-MLP 出口（fc2） | All-Reduce | 前向 | 行并行沿**收缩维**→ 部分和 | [§6](#6-mlp-的实现与计算-通信顺序) |
...
| **④ Loss** | vocab_parallel_cross_entropy | All-Reduce ×3 | 前向 | logits 切词表维，softmax 需**全局 max 与 Σexp** | [§4](#4-词嵌入并行vocabparallelembedding)、[02·Q11](./interview/02-并行化子系统面试题集.md) |
...
## 4. 词嵌入并行：VocabParallelEmbedding
...
> **Remark：为什么是 All-Reduce（求和），而不是"分块 + All-Gather"？**
>
> 直觉上 All-Gather 只需 ≈`sb·h` 通信（All-Reduce 的一半）、还免去求和，似乎更省。但这里有个结构性原因让 All-Gather **不适用**：
>
> **embedding 的输出是按「token 位置」索引的，而每张卡能算哪些位置是数据依赖、散布、数量不定的——不构成能 All-Gather 的固定分块。** All-Gather 的前提是"rank i 固定产出第 i 块、拼接即完整"（如列并行 `gather_output`）；可这里 rank i 产出的是"**恰好落在它词表段内的那些 token**"，它们散落在序列的任意位置、每卡命中几个也随 batch 变。没有"第 i 块"可拼。
>
> 看个 2 卡例子（词表 8：G0 管 id `[0,4)`、G1 管 `[4,8)`；一条序列 `[5, 2, 6, 1]`）：
>
> ```
>              位置:  p0   p1   p2   p3      (token: 5   2   6   1)
> G0 本地查表 →     [ 0   e2   0    e1 ]   ← 只有 p1,p3 命中(id 2,1)，其余清零
> G1 本地查表 →     [ e5   0   e6    0 ]   ← 只有 p0,p2 命中(id 5,6)
> ───────────────────────────────────────
> 求和(All-Reduce)→ [ e5  e2  e6   e1 ]   ✓ 每个位置恰被一张卡填、无重叠 → 相加即对
> ```
>
> **掩码清零那一步，已把每张卡的贡献「摆到了正确的稠密位置」上**，所以合并只需逐元素相加 = reduce。而"分块 All-Gather"要成立，得先把各卡命中的 token **紧凑打包**（G0=`{e2,e1}`、G1=`{e5,e6}`），All-Gather 后是**按卡拼接**的 `[e2,e1,e5,e6]`——位置全乱（应是 p1,p3,p0,p2），还得拿索引 `[1,3,0,2]` **散射回位置**；且每卡命中数不等（极端如 `[5,6,7,4]` → G0 命中 0、G1 命中 4），得用**变长 all-gather-v**。也就是说：
>
> > "分块 All-Gather" 实际 = **permute（按归属打包）+ all-gather-v（变长）+ unpermute（按索引散射回位置）**——正是 MoE dispatch 那套机器（§6.5）。
>
> **那省下的一半带宽为何不换**：① embedding 通信在**输入处只发生一次**，而 TP 的 All-Reduce 是**每层 ×L×2**，把它从 2× 压到 1× 占总量九牛一毛，却要引入变长通信 + 索引 permute/unpermute 的复杂度；② 那点求和几乎免费——All-Reduce=RS+AG，求和是通信中**边传边加**完成的（§8），且加的多是 0；③ 稠密定形 + 单集合通信 + 零索引簿记，NCCL 最优、时序确定。
>
> **框架其实已在能省时就省**：开序列并行时输出本就要沿 `s` 切，于是直接用 **Reduce-Scatter**（`layers.py:319`，≈1× 而非 2×）替代 All-Reduce（`:324`）——但它仍是 **reduce（求和）**，不是 gather，原因还是"掩码已摆位、只能相加"。
