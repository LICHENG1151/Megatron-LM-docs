# 02.2 · 张量并行实现详解（TP in Transformer）

> 本篇是 [02 · 并行化子系统](./02-并行化子系统.md) 的**子文档**，承接 [02.1 · 并行组构建与通信详解](./02.1-并行组构建与通信详解.md)。02.1 讲清了"通信组怎么建、`tp_group` 从哪来"，本篇聚焦：TP 在 Transformer 结构下**如何切分权重、列并行与行并行如何经典配对、计算与通信的精确顺序**。
>
> 相关源码：
...
> `p = tp_group.size()`，即 TP 组里的 GPU 数。`self.tp_group` 是 [02.1](./02.1-并行组构建与通信详解.md) 中由 `parallel_state` 建好并存入的那个通信组（`layers.py:864` 取得）。
...
- 触发时机：在 1F1B 调度收尾的 `finalize_model_grads` 里（见 [02.3 §5.4](./02.3-流水线并行与1F1B调度.md)），与 DP 梯度 all-reduce、embedding all-reduce 一起做。
...
- TP 与 DP 正交：TP 在 `tp_group` 内切参数本体，DistributedOptimizer 在 `dp_cp_group` 内切优化器状态（见 [02.1](./02.1-并行组构建与通信详解.md) 的 `_DATA_PARALLEL_GROUP_WITH_CP`）。
...
返回上级：[02 · 并行化子系统](./02-并行化子系统.md) ｜ 上一篇：[02.1 · 并行组构建与通信详解](./02.1-并行组构建与通信详解.md)
