# 04 · 分布式训练与优化器

本篇拆解数据并行的两条路线（自研 DDP 与 FSDP）、梯度桶机制、分布式优化器（ZeRO 式优化器状态切分），以及梯度裁剪/缩放等数值稳定性组件。

相关路径：
- `megatron/core/distributed/`
- `megatron/core/optimizer/`

---

## 1. 总体定位

模型层算出梯度后，分布式训练层负责：**在 DP 组内同步梯度**、**切分优化器状态省显存**、**更新参数**。这是数据并行（DP）维度的落地实现，与 TP/PP 正交协作。

```mermaid
flowchart TD
    M["模型 (含并行层)"] --> DDP["DistributedDataParallel<br/>梯度桶 + reduce-scatter"]
    DDP --> BUF["ParamAndGradBuffer<br/>连续显存梯度桶"]
    BUF --> OPT["DistributedOptimizer<br/>优化器状态按 DP 切分"]
    OPT --> CLIP["clip_grads 梯度裁剪"]
    OPT --> SCALE["grad_scaler 混合精度缩放"]
    OPT --> STEP["optimizer.step() 更新参数"]
    STEP --> GATHER["all-gather 更新后的参数"]
```

---

## 2. 数据并行两条路线（distributed/）

| 文件 | 职责 |
|------|------|
| `distributed_data_parallel.py` | ★ Megatron 自研 `DistributedDataParallel`（非 PyTorch 原生 DDP） |
| `distributed_data_parallel_config.py` | `DistributedDataParallelConfig`：重叠开关、桶大小、梯度精度等 |
| `param_and_grad_buffer.py` | ★ `ParamAndGradBuffer`：把参数/梯度打包进连续显存桶 |
| `data_parallel_base.py` | DP 基类 |
| `finalize_model_grads.py` | 迭代末梯度收尾：跨 PP/嵌入组的梯度处理、规约 |
| `torch_fully_sharded_data_parallel.py` | 基于 PyTorch FSDP2 的封装（FSDP 路线） |
| `reduce_scatter_with_fp32_accumulation.py` | FP32 累加的 reduce-scatter，保精度 |

### 2.1 自研 DDP + 梯度桶

为什么不用原生 DDP？因为 Megatron 需要与 TP/PP/分布式优化器深度协同、精细控制通信重叠。核心机制：

- **梯度桶（bucket）**：把众多参数的梯度拼进少数几块连续显存（`ParamAndGradBuffer`），减少 NCCL 调用次数。
- **通信-计算重叠**：反向计算某层梯度的同时，异步规约已就绪的桶（`--overlap-grad-reduce`）。
- **参数 all-gather 重叠**：与分布式优化器配合，更新后参数的 all-gather 与下一步前向重叠（`--overlap-param-gather`）。

```mermaid
flowchart LR
    subgraph 反向传播
        L3grad["层3梯度就绪"] --> RS3["异步 reduce-scatter 桶"]
        L2grad["层2梯度就绪"] --> RS2["异步 reduce-scatter 桶"]
    end
    RS3 -.重叠.-> L2grad
```

> **Remark · 梯度桶（gradient bucket）到底是什么**
>
> **是什么**：梯度桶是**每个 GPU（每个 rank）本地**的一块（或几块）连续显存缓冲区（`ParamAndGradBuffer`），把众多零散参数的梯度**首尾相接地打包**进去 —— 此后「某个参数的梯度」其实只是这块大 buffer 里的一段视图（view）。注意：桶是本地容器，**不跨卡共享**；跨 GPU 的是「桶之间的集合通信」，桶本身不跨卡。
>
> **为了解决什么问题**：
> 1. **通信次数爆炸**。一个模型有成百上千个参数，天然情况下每个参数的梯度散落各处；若逐个做 DP 同步（all-reduce / reduce-scatter），就是上千次小通信，每次都摊固定启动开销，带宽利用率极低。打成桶后**一整桶只发一次通信**，把上千次小操作合并成几次大操作。
> 2. **通信无法与计算重叠**。反向传播从后往前逐层算梯度，若没有「桶」这个边界，就无从判断「一批梯度何时算完、可以发通信了」。
>
> **怎么做的**：预先分配几块大的连续显存，把梯度按参数顺序塞进去；反向传播中某个桶覆盖的层全部算完（梯度「凑齐」）时，**立刻异步发起该桶的 reduce-scatter**，同时反向继续算前面的层，通信被藏在计算背后（`--overlap-grad-reduce`，即上图）。桶因此是**通信-计算重叠的调度单位**。
>
> **这块连续显存到底怎么存**：是**一维扁平 buffer（向量）**，不是二维矩阵、也不是保留各参数原形状的张量。前提是——PyTorch 里任何张量（无论声明成几维）在显存里本就是一段连续的一维内存 + `(shape, stride)` 元数据，「二维矩阵」只是「一维内存按行优先解释」。梯度桶正吃准这点：
> - **一维分配**：`self.grad_data = torch.zeros(self.numel, …)`，`numel` = 桶内所有参数元素数之和，`shape=(numel,)` 的一维向量。
> - **摊平紧排**：每个参数按行优先拉直，占据这条带上的一段连续区间 `param_index_map[param] = (start, end, bucket_id)`，首尾相接、下一个参数紧随其后；桶边界（`bucket_size`）只是带上的一个切分点。
> - **零拷贝还原形状**：用的时候 `grad_data[start:end].view(param.shape)`，只改元数据不搬数据，一维切片临时又「看成」二维/多维。
>
> ```text
> grad_data（一维扁平）: [ paramA 摊平 | paramB | paramC 摊平 | D | … ]
>                         0          end_A    end_B         end_C
> 逻辑视图: A→[0:end_A].view(4096,4096)  B→[end_A:end_B].view(4096)  C→…view(a,b,c)
> ```
>
> 之所以必须一维扁平：①一次 NCCL 通信只认「一个指针 + 一个长度」，摊平成连续带才能整桶一次发出；②DP 分片按 `shard_size = numel // dp_world_size` 均匀切，**切点无需对齐参数边界**——大参数可被切到两个 rank、一个分片也可横跨多个小参数，这种自由切分只有「拉平忘掉原形状」后才可能。
>
> **为何与分布式优化器天然契合**：用 `DistributedOptimizer`（ZeRO-1）时通信是 reduce-scatter —— 规约后每个 rank 只拿到结果的 `1/DP` 分片。桶是连续显存，正好沿桶均匀切成 DP 份，第 k 个 rank 拿到的分片恰好就是它要负责更新的那部分主权重，梯度/优化器状态/主权重三者的切分边界天然对齐（详见 §3.1）。
>
> 源码：`param_and_grad_buffer.py`（桶分配与 view 映射）、`distributed_data_parallel.py`（反向 hook 里判断桶就绪、触发异步通信）。

### 2.2 FSDP 路线

`torch_fully_sharded_data_parallel.py` 提供基于 PyTorch FSDP2 的替代方案；仓库还含一个自研 Megatron-FSDP（`megatron/core/distributed/fsdp/` 子包，带独立 `pyproject.toml`）。FSDP 把参数、梯度、优化器状态三者都沿 DP 切分，适合超大模型的纯数据并行场景。`examples/megatron_fsdp/` 有用例。

> FSDP(ZeRO-3) 的原理（显存 `16P→16P/N`、通信 ≈1.5× DP、flat buffer + 预取重叠）与两套实现开关的完整讲解，见铺垫专题 [02.3.1 · FSDP](./02.3.1-FSDP.md)。

---

## 3. 优化器子系统（optimizer/）

| 文件 | 职责 |
|------|------|
| `optimizer.py` | 优化器基类与混合精度优化器封装 |
| `distrib_optimizer.py` | ★ `DistributedOptimizer`：ZeRO-1 式优化器状态切分 |
| `optimizer_config.py` | `OptimizerConfig`：lr、weight decay、精度等 |
| `clip_grads.py` | 全局梯度范数裁剪（跨所有并行组规约范数） |
| `grad_scaler.py` | 混合精度（FP16）动态损失缩放 |
| `layer_wise_optimizer.py` | 逐层优化器（与梯度桶路由配合） |
| `muon.py` / `emerging_optimizers.py` | Muon 等新兴优化器接入 |
| `qk_clip.py` | QK 裁剪（注意力稳定性） |
| `cpu_offloading/` | 优化器状态/计算卸载到 CPU |
| `optimizer_cuda_graph.py` | 优化器步骤的 CUDA Graph 加速 |

### 3.1 DistributedOptimizer（关键）

把优化器状态（如 Adam 的 m、v）沿 **DP 维度切分**，每个 DP rank 只持有 `1/DP` 的优化器状态与主权重，显著省显存（ZeRO-1 思想）。

```mermaid
flowchart TD
    GRAD["完整梯度桶"] --> RS["reduce-scatter<br/>每 rank 只得到自己负责的梯度分片"]
    RS --> LOCALOPT["本地 optimizer.step()<br/>只更新本分片的主权重 (FP32)"]
    LOCALOPT --> AG["all-gather<br/>把更新后的权重广播回所有 DP rank"]
    AG --> MODEL["模型权重 (BF16/FP16) 更新完成"]
```

- 主权重以 FP32 保存（数值稳定），模型计算用 BF16/FP16。
- 与梯度桶天然契合：reduce-scatter 的输出分片正好是本 rank 要更新的部分。
- 与 `--overlap-param-gather` 配合，AG 与下个前向重叠。

### 3.2 梯度收尾与裁剪

- `finalize_model_grads.py`：处理 PP 各 stage、嵌入权重共享组的梯度同步，确保 tied 权重梯度一致。
- `clip_grads.py`：先跨 TP/PP/DP 规约出全局梯度范数，再统一缩放，保证裁剪在并行下语义正确。

---

## 4. 混合精度

Megatron 支持 FP16 / BF16 / FP8 / FP4（见 `core/fp8_utils.py`、`fp4_utils.py`）：

- **FP16**：需 `grad_scaler.py` 动态损失缩放防下溢。
- **BF16**：动态范围大，通常无需损失缩放。
- **FP8**：依赖 Transformer Engine，在 GEMM 处用 FP8 加速（Hopper/Blackwell）。
- 主权重恒为 FP32（在分布式优化器内维护）。

---

## 5. 与其他子系统的协作

```mermaid
flowchart LR
    PP["pipeline schedules (02)"] -->|最后微批触发梯度同步| DDP["DDP (本篇)"]
    DDP --> OPT["DistributedOptimizer (本篇)"]
    OPT -->|DP 组| MPU["parallel_state (02)"]
    OPT --> CKPT["dist_checkpointing (08)<br/>优化器状态分片落盘"]
    TRAIN["training.train_step (06)"] --> OPT
```

- PP 调度只在每个 global batch 的最后一个微批做梯度规约（其余微批关闭 grad sync）。
- 优化器状态分片直接映射到分布式检查点的分片格式（见 [08](./08-检查点与重切分.md)）。
- 训练主循环 `train_step()` 编排「前向反向 → 优化器步进 → LR 调度」。

下一篇：[数据集与分词器](./05-数据集与分词器.md)。
